...
function getShoppingLists(res, userID, connection, context, complete) {
    ...
    var query = 'SELECT listID, nameList, CAST(listCreated as DATE) AS listCreated FROM Users LEFT JOIN Lists ON Lists.userID = Users.userID WHERE Users.userID=?';
...
function getShoppingListByID(res, listID, connection, context, complete) {
  ...
  var query = 'SELECT listID, nameList, CAST(listCreated AS DATE) as listCreated FROM Lists WHERE Lists.listID=?';
